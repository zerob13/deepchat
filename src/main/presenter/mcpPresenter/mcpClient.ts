...
export class McpClient {
  ...
  async callTool(toolName: string, args: Record<string, unknown>): Promise<string> {
    ...
    try {
      ...
      // 调用工具
      const result = await this.client.callTool({ name: toolName, arguments: processedArgs }) as ToolCallResult;
  ...
  async readResource(resourceUri: string): Promise<Resource> {
    ...
    try {
      ...
      const resource: Resource = {
        uri: resourceUri,
        text: typeof rawResource === 'object' && rawResource !== null && 'text' in rawResource
...
export async function createMcpClient(serverName: string): Promise<McpClient> {
  // 读取 MCP 配置
  const configPath = path.join(app.getAppPath(), 'resources', 'mcp', 'mcpConfig.json');
  ...
  // 创建并返回 MCP 客户端
  return new McpClient(serverName, serverConfig);
}
...
export async function getDefaultMcpClient(): Promise<McpClient> {
  ...
  // 获取默认服务器名称
  const defaultServerName = configData.defaultServer;
