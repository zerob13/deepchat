...
import type { ChannelAdapterConfig } from './runtime/types'
import { resolveAcpAgentAlias } from '@shared/utils/acpAgentAlias'
import { REMOTE_CONTROL_ERROR_MESSAGES } from '@shared/contracts/remoteControlErrors'
import type { RemoteServiceDeps } from './ports'
...
import { ChannelManager } from './runtime/manager'
import { TelegramAdapter } from './channels/telegram/adapter'
import { DiscordAdapter } from './channels/discord/adapter'
import { FeishuAdapter } from './channels/feishu/adapter'
import { QQBotAdapter } from './channels/qqbot/adapter'
import { WeixinIlinkAdapter } from './channels/weixinIlink/adapter'
import {
...
} from './channels/feishu/feishuAuth'
import { WeixinIlinkClient } from './channels/weixinIlink/weixinIlinkClient'
import {
  REMOTE_DELIVERY_CHANNELS,
  RemoteDelivery,
  type RemoteDeliveryInput
...
export class RemoteService {
  ...
  private readonly delivery: RemoteDelivery
  ...
  async deliverCronJobResult(
    ...
    return this.enqueueRuntimeOperation(() => this.delivery.deliver(input))
  }
  ...
  private async rebuildWeixinIlinkAccountRuntime(accountId: string): Promise<void> {
    ...
    try {
      ...
      logger.warn('[RemoteService] Failed to connect Weixin iLink adapter.', {
        accountId: account.accountId,
        error
      })
      // The adapter status snapshot already captures the failure.
...
<html>
  ...
  private logWeixinTrace(message: string, context?: Record<string, unknown>): void {
    ...
    logger.info(`[RemoteService] ${message}`, context)
  }
  ...
  private async registerTelegramCommands(client: TelegramClient): Promise<void> {
    try {
      ...
      console.warn('[RemoteService] Failed to register Telegram commands:', error)
    }
  }
}
